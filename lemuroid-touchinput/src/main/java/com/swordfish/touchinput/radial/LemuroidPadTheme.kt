...
class LemuroidPadTheme {
    ...
    fun compositeFill(pressed: Boolean): Color = if (pressed) compositeFillPressed else compositeFill

    fun foregroundFill(pressed: Boolean): Color = if (pressed) buttonFillPressed else buttonFill

    fun icons(pressed: Boolean): Color = if (pressed) iconColorPressed else iconColor
}
